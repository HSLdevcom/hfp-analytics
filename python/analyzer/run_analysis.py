    ...
    try:
        ...
        for vehicle in vehicles:
            ...
            try:
                ...
                combined_obj = {}
                for obj in chain(
                    analyzed_door_data, analyzed_odo_data, analyzed_positioning_data
                ):
                ...
                logger.exception(
                    f"Error while analyzing vehicle: {vehicle_operator_id}/{vehicle_number}"
                )
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                logger.debug(f"{get_time()} {cur.fetchone()[0]} observations inserted.")
                ...
                cur.execute(
                    "SELECT * FROM stopcorr.guess_missing_stop_ids(%s)",
                    (STOP_NEAR_LIMIT_M,),
                )
                logger.debug(
                    f"{get_time()} {cur.fetchone()[0]} observations updated with guessed stop_id"
                )

                cur.execute(
                    "SELECT stop_id FROM stopcorr.observed_stop_not_in_jore_stop"
                )
                res = [str(x[0]) for x in cur.fetchall()]
                n_stops = len(res)
                if n_stops > 10:
                    logger.debug(
                        f'{n_stops} stop_id values in "observation" not found in "jore_stop"'
                    )
                elif n_stops > 0:
                    stops_str = ", ".join(res)
                    logger.debug(
                        f'stop_id values in "observation" not found in "jore_stop": {stops_str}'
                    )

                cur.execute("SELECT * FROM stopcorr.calculate_jore_distances()")
                logger.debug(
                    f"{get_time()} {cur.fetchone()[0]} observations updated with dist_to_jore_point_m"
                )

                cur.execute(
                    "WITH deleted AS (DELETE FROM stopcorr.stop_median RETURNING 1)\
                            SELECT count(*) FROM deleted"
                )
                logger.debug(
                    f'{get_time()} {cur.fetchone()[0]} rows deleted from "stop_median"'
                )

                cur.execute(
                    "SELECT * FROM stopcorr.calculate_medians(%s, %s)",
                    (MIN_OBSERVATIONS_PER_STOP, MAX_NULL_STOP_DIST_M),
                )
                logger.debug(
                    f'{get_time()} {cur.fetchone()[0]} rows inserted into "stop_median"'
                )

                cur.execute("SELECT * FROM stopcorr.calculate_median_distances()")
                logger.debug(
                    f"{get_time()} {cur.fetchone()[0]} observations updated with dist_to_median_point_m"
                )

                cur.execute(
                    "SELECT * FROM stopcorr.calculate_percentile_radii(%s)",
                    (RADIUS_PERCENTILES,),
                )
                logger.debug(
                    f'{get_time()} {cur.fetchone()[0]} "percentile_radii" created using percentiles {RADIUS_PERCENTILES}'
                )

                cur.execute(
                    "CALL stopcorr.classify_medians(%s, %s, %s, %s, %s, %s, %s, %s)",
                    (
                        MIN_RADIUS_PERCENTILES_TO_SUM,
                        DEFAULT_MIN_RADIUS_M,
                        MANUAL_ACCEPTANCE_MIN_RADIUS_M,
                ...
                cur.execute(
                    "SELECT count(*) FROM stopcorr.stop_median WHERE result_class IS NOT NULL"
                )
                logger.debug(
                    f'{get_time()} {cur.fetchone()[0]} "stop_median" updated with "result_class", "recommended_min_radius_m" and "manual_acceptance_needed"'
                )

                cur.execute("CALL staging.remove_accidental_signins()")
                logger.debug(
                    "Accidental signins removed from api.assumed_monitored_vehicle_journey"
                )

                duration = time.time() - start_time
                logger.debug(
                    f"{get_time()} Analysis complete in {int(duration)} seconds."
                )
