    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "DELETE FROM stopcorr.observation WHERE oday < now() - interval '3 week'"
                )
                logger.debug(f"{cur.rowcount} rows deleted from observation.")

                logger.debug("Removing journey data older than 3 weeks.")
                cur.execute(
                    "DELETE FROM hfp.assumed_monitored_vehicle_journey WHERE oday < now() - interval '3 week'"
                )
                logger.debug(
                    f"{cur.rowcount} rows deleted from assumed monitored vehicle journeys."
                )
                ...
                cur.execute(
                    "DELETE FROM logs.importer_log WHERE log_timestamp < now() - interval '4 week'"
                )
