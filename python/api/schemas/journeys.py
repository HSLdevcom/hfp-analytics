...
class Journey(BaseModel):
    route_id: str = Field(title="Route ID", description="`route_id` in HFP topic", examples=["4611"])
    direction_id: Literal[1, 2] = Field(title="Direction ID", description="`direction_id` in HFP topic", examples=["1"])
    oday: date = Field(title="Operating day", description="`oday` in HFP payload", examples=["2023-03-18"])
    start: time = Field(title="Start time of the journey", description="`start` in HFP payload", examples=["10:12:00"])
    operator_id: int = Field(
        title="Operator owning the vehicle", description="`operator_id` in HFP topic", examples=[12]
    )
    oper: int = Field(title="Operator running the journey", description="`oper` in HFP payload", examples=[12])
    vehicle_number: int = Field(title="Vehicle number", description="`vehicle_number` in HFP topic", examples=["114"])
    transport_mode: TransportModeEnum = Field(
        title="The type of the vehicle", description="`transport_mode` in HFP topic", examples=["bus", "tram"]
    )
