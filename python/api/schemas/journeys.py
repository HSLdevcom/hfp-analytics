...
class Journey(BaseModel):
    route_id: str = Field(title="Route ID", description="`route_id` in HFP payload", example="4611")
    direction_id: int = Field(title="Direction ID", description="`direction_id` in HFP payload", example="1")
    oday: date = Field(title="Operating day", description="`oday` in HFP payload", example="2023-03-18")
    start_24h: time = Field(title="Start time of the journey", description="`start` in HFP payload", example="10:12:00")
    operator_id: int = Field(title="Operator owning the vehicle", description="`operator_id` in HFP topic", example=12)
    vehicle_operator_id: int = Field(
    ...
    min_timestamp: datetime = Field(
        ...
        description="The first timestamp of the events of the journey.",
        example="2023-03-18T18:26:00.000",
    )
    max_timestamp: datetime = Field(
        title="Maximum timestamp of events",
        description="The last timestamp of the events of the journey.",
