...
async def get_vehicle_ids(
    date: date, customTimeInterval=None, operator_id=None
) -> list:
    """Query all vehicles filtered by date and return them as a list of dicts"""
    date_str = str(date)
    query_params = {
        "start": datetime.strptime(
            date_str + " 00:00:00.000+00", "%Y-%m-%d %H:%M:%S.%f+00"
        ),
        "end": datetime.strptime(
            date_str + " 11:59:00.000+00", "%Y-%m-%d %H:%M:%S.%f+00"
        ),
    }
    ...
    if customTimeInterval:
        query_params = {
            "start": datetime.strptime(
                date_str + customTimeInterval["start"], "%Y-%m-%d %H:%M:%S.%f+00"
            ),
            "end": datetime.strptime(
                date_str + customTimeInterval["end"], "%Y-%m-%d %H:%M:%S.%f+00"
            ),
        }
...
async def get_vehicle_data(date, operator_id, vehicle_number, customTimeInterval=None):
    ...
    vehicle_data = await get_vehicles_by_timestamp(
        date, vehicle_operator_id, vehicle_number, customTimeInterval
    )
    grouped_data = {}
...
def analyze_positioning_data(vehicle_data):
    analysis = defaultdict(
        lambda: {
            ...
            "loc_gps": 0,
            "loc_dr": 0,
            "loc_error_events": {"amount": 0, "events": []},
        }
    )
    ...
    for vehicle_number, analysis_data in analysis.items():
        total = sum(
            [
                ...
                if key in ["loc_null", "loc_gps", "loc_dr"]
            ]
        )
...
def analyze_vehicle_door_data(vehicle_data):
    analysis = defaultdict(
        lambda: {
            ...
            "true": 0,
            "false": 0,
            "door_error_events": {"amount": 0, "events": []},
        }
    )
    for data in vehicle_data:
        data_list = data.get("data", [])
            ...
            result.append(
                {
                    ...
                    "events_amount": None,
                    "door_error_types": [],
                    "door_error_events": {"events": [], "types": []},
                }
            )
            continue
...
def analyze_odo_data(vehicle_data):
    analysis = defaultdict(
        lambda: {"odo": 0, "null": 0, "odo_error_events": {"amount": 0, "events": []}}
    )
    for data in vehicle_data:
        data_list = data.get("data", [])
        data_list_sorted = sorted(data_list, key=lambda x: x["tst"])
        ...
        if firstEvent is not None and lastEvent is not None:
            ...
            if firstOdo == lastOdo:
                analysis[data["vehicle_number"]]["odo_error_events"]["events"].append(
                    error_obj(firstEvent, "Identical first and last odo values")
                )
                analysis[data["vehicle_number"]]["odo_error_events"]["events"].append(
                    error_obj(lastEvent, "Identical first and last odo values")
                )
            if lastOdo is not None and lastOdo > 1000000:
                analysis[data["vehicle_number"]]["odo_error_events"]["events"].append(
                    error_obj(lastEvent, "Odo value over 1000000")
                )
        ...
        for d in data_list_sorted:
            ...
            if all((prevOdo, odo, previousEvent)):
                ...
                if prevOdo > odo and d.get("start") == previousEvent.get("start"):
                    analysis[data["vehicle_number"]]["odo_error_events"][
                        "events"
                    ].append(error_obj(previousEvent, "Odo value decreased"))
                    analysis[data["vehicle_number"]]["odo_error_events"][
                        "events"
                    ].append(error_obj(d, "Odo value decreased"))
        ...
        for stationaryChunk in stationaryEventChunks:
            ...
            if stationaryChunkLength >= stationary_odo_events_threshold:
                firstOdo = stationaryChunk[stationary_odo_events_threshold - 1].get(
                    "odo"
                )
                ...
                if firstOdo is not None and lastOdo is not None and firstOdo != lastOdo:
                    analysis[data["vehicle_number"]]["odo_error_events"][
                        ...
                        error_obj(stationaryChunk[0], "Odo changed when stationary")
                    )
                    analysis[data["vehicle_number"]]["odo_error_events"][
                        "events"
                    ].append(
                        error_obj(
                            stationaryChunk[stationaryChunkLength - 1],
                            "Odo changed when stationary",
                        )
                    )
    ...
    for vehicle_number, analysis_data in analysis.items():
        total = sum(
            [analysis_data[key] for key in analysis_data if key in ["odo", "null"]]
        )
            ...
            data = [
                {
                    ...
                    "loc_gps_ratio": r[5],
                    "loc_dr_ratio": r[6],
                    "loc_error_events": {"events": r[7], "types": r[8]},
                }
                for r in rows
            ]
            return data
            ...
            data = [
                {
                    ...
                    "door_error_events": {"events": r[7], "types": r[8]},
                }
                for r in rows
            ]
            return data
            ...
            data = [
                {
                    ...
                    "odo_null_ratio": r[5],
                    "odo_error_events": {"events": r[6], "types": r[7]},
                }
                for r in rows
            ]
            return data
            ...
            data = [
                {
                    ...
                    "date": r[2],
                    "events_amount": r[3],
                    "door_error_events": {"events": r[4], "types": r[5]},
                    "odo_error_events": {"events": r[6], "types": r[7]},
                    "loc_error_events": {"events": r[8], "types": r[9]},
                }
                for r in rows
            ]
            return data
...
async def get_vehicles_by_timestamp(
    date: date, vehicle_operator_id: int, vehicle_number=None, customTimeInterval=None
) -> list:
                ...
                try:
                    await cur.execute(
                        ...
                        (
                            ...
                            data["operator_id"],
                            data["date"],
                            data["drst_null_ratio"],
                            data["drst_true_ratio"],
                            data["drst_false_ratio"],
                            json.dumps(
                                data["door_error_events"]["events"], cls=DateTimeEncoder
                            ),
                            data["door_error_events"]["types"],
                            data["odo_exists_ratio"],
                            data["odo_null_ratio"],
                            json.dumps(
                                data["odo_error_events"]["events"], cls=DateTimeEncoder
                            ),
                            ...
                            data["loc_dr_ratio"],
                            json.dumps(
                                data["loc_error_events"]["events"], cls=DateTimeEncoder
                            ),
                    ...
                    print(
                        f"Error: {e}. Skipping row with date={data['date']} and vehicle_number={data['vehicle_number']}"
                    )
                    continue
            await conn.commit()
            return "Inserts done"
