...
def get_env(
    var_name: str, default_value: Union[str, None] = None, modifier: Callable = dummy
):
    ...
    if not env:
        if default_value is not None:
            env = default_value
            logging.warning(
                f"{var_name} not set in env, falling back to default value {default_value}"
            )
        else:
...
BUILD_VERSION: str = get_env(
    "BUILD_VERSION", "Build NaN"
)  # Azure pipeline will give this as a build argument.
...
SLACK_USERS_TO_ALERT: str = get_env(
...
MIN_OBSERVATIONS_PER_STOP: int = get_env(
    "MIN_OBSERVATIONS_PER_STOP", "10", modifier=env_as_int
)
MAX_NULL_STOP_DIST_M: float = get_env(
    "MAX_NULL_STOP_DIST_M", "100.0", modifier=env_as_float
)
RADIUS_PERCENTILES: list[float] = get_env(
    "RADIUS_PERCENTILES", "0.5,0.75,0.9,0.95", modifier=env_as_float_list
)
MIN_RADIUS_PERCENTILES_TO_SUM: list[float] = get_env(
    "MIN_RADIUS_PERCENTILES_TO_SUM", "0.5,0.95", modifier=env_as_float_list
)
DEFAULT_MIN_RADIUS_M: float = get_env(
    "DEFAULT_MIN_RADIUS_M", "20.0", modifier=env_as_float
)
MANUAL_ACCEPTANCE_MIN_RADIUS_M: float = get_env(
    "MANUAL_ACCEPTANCE_MIN_RADIUS_M", "40.0", modifier=env_as_float
)
LARGE_SCATTER_PERCENTILE: float = get_env(
    "LARGE_SCATTER_PERCENTILE", "0.9", modifier=env_as_float
)
LARGE_SCATTER_RADIUS_M: float = get_env(
    "LARGE_SCATTER_RADIUS_M", "10.0", modifier=env_as_float
)
LARGE_JORE_DIST_M: float = get_env("LARGE_JORE_DIST_M", "25.0", modifier=env_as_float)
STOP_GUESSED_PERCENTAGE: float = get_env(
    "STOP_GUESSED_PERCENTAGE", "0.05", modifier=env_as_float
)
TERMINAL_IDS: list[int] = get_env(
    "TERMINAL_IDS",
    "1000001,1000015,2000002,2000003,2000212,4000011",
    modifier=env_as_int_list,
)
