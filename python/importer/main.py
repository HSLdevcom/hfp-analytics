...
from .schemas import APC as APCSchema, HFP as HFPSchema, TLR as TLRSchema
...
importers = {
    ...
    "TLR": Importer(HFP_STORAGE_CONTAINER_NAME, data_converter=zst_csv_to_dict_decoder, db_schema=TLRSchema),
}
    ...
    for importer_type, importer in importers.items():
        ...
        while import_date <= datetime.now():
            for blob_name in importer.list_blobs_for_date(import_date):
                ...
                blob_data["event_type"] = metadata.get("eventType") if importer_type in ["HFP", "TLR"] else "APC"
...
def import_blob(blob_name):
    ...
    try:
        ...
        importer = importers["APC"] if importer_type == "APC" else importers["TLR"] if importer_type == "TLR" else importers["HFP"]
