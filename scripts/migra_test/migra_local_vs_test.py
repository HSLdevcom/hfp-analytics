...
def is_test_db_connection_string_ok(test_db_connection_string):
    if test_db_connection_string is None:
        print(
            "TEST_DB_CONNECTION_STRING was not found, do you have it defined in .env file?"
        )
        return False
    ...
    if len(args) > 1:
        ...
        if arg == "print" or arg == "init":
            with (
                ...
                S(test_db_connection_string) as s_test,
            ):
                m = Migration(
                    x_from=s_test,
                    x_target=s_local,
        ...
        if arg == "apply":
            print(
                f"Do you want to apply changes to test from >>> {DEPLOY_FILE_NAME} <<< file?"
            )
